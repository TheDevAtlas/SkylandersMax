...
void HavokImport::DoImport(const std::string &fileName, bool suppressPrompts) {
  ...
  for (auto &v : *rootCont) {
    if (v == hkaAnimationContainer::GetHash()) {
      ...
      // SetMotionIndex(35); //Setting the index works
      // motionIndex++; //Iterating does not work here, it needs to go in HavokMax.cpp when the UpdatePresetUI function is called
